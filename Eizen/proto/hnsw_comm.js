...
$root.index_buffer = (() => {
	...
	index_buffer.LayerNode = (() => {
		...
		LayerNode.encode = function encode(message, writer) {
			...
			if (message.level != null && Object.hasOwn(message, "level"))
				writer.uint32(/* id 1, wireType 0 =*/ 8).uint32(message.level);
			if (message.idx != null && Object.hasOwn(message, "idx"))
				writer.uint32(/* id 2, wireType 0 =*/ 16).uint32(message.idx);
			if (message.visible != null && Object.hasOwn(message, "visible"))
				writer.uint32(/* id 3, wireType 0 =*/ 24).bool(message.visible);
			if (message.neighbors != null && Object.hasOwn(message, "neighbors"))
				for (
					var keys = Object.keys(message.neighbors), i = 0;
					i < keys.length;
					++i
				)
					writer
						...
						.float(message.neighbors[keys[i]])
						.ldelim();
			return writer;
		};
		 ...
		 */
		LayerNode.decode = function decode(reader, length, error) {
			if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
			var end = length === undefined ? reader.len : reader.pos + length,
				message = new $root.index_buffer.LayerNode(),
				key,
				value;
		...
		LayerNode.fromObject = function fromObject(object) {
			...
			if (object.neighbors) {
				...
				message.neighbors = {};
				for (
					var keys = Object.keys(object.neighbors), i = 0;
					i < keys.length;
					++i
				)
		...
		LayerNode.toObject = function toObject(message, options) {
			...
			if (
				message.neighbors &&
				(keys2 = Object.keys(message.neighbors)).length
			) {
				object.neighbors = {};
				for (var j = 0; j < keys2.length; ++j)
					object.neighbors[keys2[j]] =
						options.json && !isFinite(message.neighbors[keys2[j]])
							? String(message.neighbors[keys2[j]])
							: message.neighbors[keys2[j]];
			}
			return object;
		};
	...
	index_buffer.Point = (() => {
		...
		Point.decode = function decode(reader, length, error) {
			if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
			var end = length === undefined ? reader.len : reader.pos + length,
				message = new $root.index_buffer.Point();
			while (reader.pos < end) {
				var tag = reader.uint32();
				if (tag === error) break;
				switch (tag >>> 3) {
		...
		Point.verify = function verify(message) {
			...
			if (message.idx != null && Object.hasOwn(message, "idx"))
				if (!$util.isInteger(message.idx)) return "idx: integer expected";
			if (message.v != null && Object.hasOwn(message, "v")) {
				if (!Array.isArray(message.v)) return "v: array expected";
				for (var i = 0; i < message.v.length; ++i)
					if (typeof message.v[i] !== "number") return "v: number[] expected";
			}
			return null;
		};
		...
		Point.toObject = function toObject(message, options) {
			...
			if (message.v && message.v.length) {
				...
				for (var j = 0; j < message.v.length; ++j)
					object.v[j] =
						options.json && !isFinite(message.v[j])
							? String(message.v[j])
							: message.v[j];
			}
			return object;
	...
	index_buffer.PointQuant = (() => {
		 ...
		 * @throws {$protobuf.util.ProtocolError} If required fields are missing
		 */
		PointQuant.decode = function decode(reader, length, error) {
			if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
			var end = length === undefined ? reader.len : reader.pos + length,
				message = new $root.index_buffer.PointQuant();
			while (reader.pos < end) {
				var tag = reader.uint32();
				if (tag === error) break;
				switch (tag >>> 3) {
		...
		PointQuant.verify = function verify(message) {
			...
			if (message.v != null && Object.hasOwn(message, "v")) {
				if (!Array.isArray(message.v)) return "v: array expected";
				for (var i = 0; i < message.v.length; ++i)
					if (!$util.isInteger(message.v[i])) return "v: integer[] expected";
			}
			return null;
		};
