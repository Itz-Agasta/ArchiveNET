...
$root.request_buffer = (() => {
	...
	request_buffer.Singleton = (() => {
		...
		Singleton.encode = function encode(message, writer) {
			...
			if (message.metadata != null && Object.hasOwn(message, "metadata"))
				for (
					var keys = Object.keys(message.metadata), i = 0;
					i < keys.length;
					++i
				)
					writer
		...
		Singleton.decode = function decode(reader, length, error) {
			...
			var end = length === undefined ? reader.len : reader.pos + length,
				message = new $root.request_buffer.Singleton(),
				key,
				value;
		...
		Singleton.fromObject = function fromObject(object) {
			if (object instanceof $root.request_buffer.Singleton) return object;
			...
			if (object.metadata) {
				if (typeof object.metadata !== "object")
					throw TypeError(
						".request_buffer.Singleton.metadata: object expected",
					);
				message.metadata = {};
				for (
					var keys = Object.keys(object.metadata), i = 0;
					i < keys.length;
					++i
				)
	...
	request_buffer.Batch = (() => {
		...
		Batch.decode = function decode(reader, length, error) {
			if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
			var end = length === undefined ? reader.len : reader.pos + length,
				message = new $root.request_buffer.Batch();
			while (reader.pos < end) {
				var tag = reader.uint32();
				if (tag === error) break;
				switch (tag >>> 3) {
		...
		Batch.verify = function verify(message) {
			...
			if (message.b != null && Object.hasOwn(message, "b")) {
				...
				for (var i = 0; i < message.b.length; ++i)
					if (!$util.isString(message.b[i])) return "b: string[] expected";
			}
			return null;
		};
		...
		Batch.fromObject = function fromObject(object) {
			if (object instanceof $root.request_buffer.Batch) return object;
		...
		Batch.toObject = function toObject(message, options) {
			if (!options) options = {};
			var object = {};
			if (options.arrays || options.defaults) object.b = [];
